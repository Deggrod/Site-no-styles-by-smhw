class AHPCalculator {
  ...
  renderCriteriaList() {
    ...
    document.querySelectorAll('.remove-criterion').forEach((btn, index) => {
  ...
  displayResults(isConsistent) {
    ...
    const isConsistentBool = isConsistent === 1; // Преобразуем обратно в boolean если нужно
    
    consistencyDiv.innerHTML = `
      <div class="consistency-message ${isConsistentBool ? 'success' : 'warning'}">
        <i class="fas fa-${isConsistentBool ? 'check-circle' : 'exclamation-triangle'}"></i>
        <p>${isConsistentBool ? 
          'Матрица согласована (CR ≤ 0.1)' : 
          'Внимание: матрица несогласована (CR > 0.1). Рекомендуется пересмотреть оценки!'}</p>
      </div>
      <div class="consistency-value">
        Коэффициент согласованности: <strong>${this.consistencyRatio.toFixed(3)}</strong>
      </div>
    `;
}
...
document.addEventListener('DOMContentLoaded', () => {
  const ahp = new AHPCalculator();
  
  // Добавляем обработчик для кнопки рекомендации треков
  document.getElementById('suggest-tracks-btn').addEventListener('click', () => {
    ahp.suggestTracks();
  });
});